...
rabbit-insert-test-messages: ## Insert messages in defined queue
	docker-compose exec rabbitmq bash -c "/opt/resources/publisher_test.sh"

rabbit-insert-app-messages: ## Insert messages in defined queue
	docker-compose exec rabbitmq bash -c "/opt/resources/publisher_app.sh"

clickhouse-read-entries: ## Read from the materialized view
...
clickhouse-read-events: ## Read from the materialized view
	docker-compose exec clickhouse bash -c "clickhouse-client --query=\"SELECT * FROM dw.events LIMIT 10 FORMAT PrettyCompactMonoBlock;\""
